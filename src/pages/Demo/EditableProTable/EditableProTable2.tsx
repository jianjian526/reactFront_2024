import { getTableData } from '@/services/DemoSvc/EditableProTable2Services';
import type {
  ActionType,
...
const EditableProTable2: React.FC = () => {
  const [editableKeys, setEditableRowKeys] = useState<React.Key[]>(() => []);
  const formRef = useRef<ProFormInstance<any>>();
  const actionRef = useRef<ActionType>();
  const editableFormRef = useRef<EditableFormInstance>();

  var defaultData: DataSourceType[] = [];

  const fetchData = async () => {
    defaultData = await getTableData();

    return { table: defaultData.data.list };
  };

  useEffect(() => {
    // const fetchData = async () => {
    //     const result = await getTableData();
    //     defaultData = result.data.list;
    // };
    // fetchData();
    // 初期值在这里设定无效
  }, []);
      ...
      >
        ...
        >
          <ProFormDependency name={['table']}>
            {({ table }) => {
              console.log(
                '=======++++' + JSON.stringify(table as DataSourceType[]),
              );
